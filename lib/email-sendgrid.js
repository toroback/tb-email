    ...
    return new Promise((resolve, reject) =>{
      ...
      createMailData(data)
        ...
        .then(res =>{
          ...
          let to  = assertReceiversArray(mailData.to).map(item => {return {id: id, status: status};});
          let cc  = assertReceiversArray(mailData.cc).map(item => {return  {id: id, status: status};});
          let bcc  = assertReceiversArray(mailData.bcc).map(item => {return  {id: id, status: status};});
...
function createMailData(data){
  return new Promise((resolve, reject) =>{
    ...
    mail.substitutions = data.substitutions;
